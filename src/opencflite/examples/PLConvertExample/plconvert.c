...
#if defined(_WIN32)
   HANDLE hFile = CreateFileA(fname, GENERIC_WRITE, 0, 0, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0); 
   int fd = _open_osfhandle((intptr_t)hFile, O_WRONLY | O_CREAT | O_TRUNC);
#else
    ...
    fsync(fd);
#endif
    close(fd); // Also closes hFile if it exists
