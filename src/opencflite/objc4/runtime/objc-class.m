...
void object_setIvar(id obj, Ivar ivar, id value)
{
    if (obj && ivar) {
        ...
        if (_class_usesAutomaticRetainRelease(cls)) {
            ...
            strong_layout = class_getIvarLayout(cls);
    ...
    if (obj  &&  ivar) {
        ...
        if (_class_usesAutomaticRetainRelease(cls)) {
            ...
            if (weak_layout && is_scanned_offset(ivar_offset - instanceStart, weak_layout)) {
                ...
                id *location = (id *)((char *)obj + ivar_offset);
                return 0; //objc_loadWeak(location);
            }
        }
        idx = (id *)((char *)obj + ivar_offset);
#if SUPPORT_GC
        if (UseGC) {
            const uint8_t *weak_layout = class_getWeakIvarLayout(cls);
            if (weak_layout && is_scanned_offset(ivar_offset, weak_layout)) {
                               ...
                               id *results, unsigned num_requested)
{
    ...
    shift = 0;
    ctor = _class_hasCxxStructors(cls);
    for (i = 0; i < num_allocated; i++) {
        id obj = results[i];
...
{
	objc_property_attribute_t *result = 0;
    size_t size = 0;
    unsigned int attrcount = 1;
    const char *s;
    objc_property_attribute_t *ra = 0;
    char *rs = 0;
