...
{
    struct old_protocol *result = 0;
    struct old_protocol_ext *ext = 0;
...
protocol_addProtocol(Protocol *proto_gen, Protocol *addition_gen) 
{
    ...
    struct old_protocol_list *protolist = 0;
...
_protocol_addMethod(struct objc_method_description_list **list, SEL name, const char *types)
{
    ...
    desc = &(*list)->list[(*list)->count++];
    desc->name = name;
    desc->types = _strdup_internal(types ? types : "");
}
                      ...
                      unsigned int count)
{
    ...
    prop = property_list_nth(*plist, (*plist)->count++);
...
Class gdb_class_getClass(Class cls)
{
    ...
    rCls = look_up_class(className, NO, NO);
    return rCls;
